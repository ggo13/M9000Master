#!/bin/bash
mysql -u root -pusi < MySql-Station-upgrade-remote.sql 2>&1 | grep -v "\[Warning\] Using a password"
...
#	mysql -uroot -pusi < update_users_Auth_plugin.sql
#	if [ $? -eq 0 ]
#	then
#		echo "Successfully updated authentication plugin for user dfr"
#	else
#		echo "Error in updating authentication plugin  for user dfr"
#	exit 1;
#	fi
