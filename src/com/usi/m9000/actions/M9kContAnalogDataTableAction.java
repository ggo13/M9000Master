...
public class M9kContAnalogDataTableAction extends ActionSupport implements SessionAware,ServletRequestAware,ServletResponseAware {
	...
	static org.apache.logging.log4j.Logger logger = org.apache.logging.log4j.LogManager.getLogger(M9kContAnalogDataTableAction.class);
	...
	//JQGrid implementation
	  ...
	  private String id;
	  private String analogStartTime;
	  private String analogEndTime;
	  private static final String contDataType = M9kConstants.ANALOG;
	  private String statusMessage="";
	public M9kContAnalogDataTableAction() {
	...
	public String getContAnalogData() throws M9000Exception
	{
		try {
			logger.debug("Entered getContAnalogData with stationId "+stationId+" lt type "+contDataType);
			if (getStationId() == null)
			{
				stationId = (String) session.get("stationId");
			}
			logger.debug("Stilllllll ContAnalog DATA : Page " + getPage() + " Rows " + getRows()
...
//		      total = (int) Math.ceil((double) records / (double) rows);
    ...
    public String showContinuousAnalogData()
	{
    	String returnStatus = SUCCESS;
		try {
			logger.debug("BEfore conversion "+analogStartTime+" end "+analogEndTime);
			String convertedStartDate = M9kUtils.convertDateToSearch(getAnalogStartTime(),"MM/dd/yyyy HH:mm:ss","yyyy-MM-dd HH:mm:ss");
			String convertedEndDate = M9kUtils.convertDateToSearch(getAnalogEndTime(),"MM/dd/yyyy HH:mm:ss","yyyy-MM-dd HH:mm:ss");
			...
			messageProperties.addProperty("DATA_TYPE", M9kConstants.CONTINUOUS_ANALOG_DATA);
...
//			}
			...
			e1.printStackTrace();
			logger.error("Error occured while creating the continuouos data file",e1);
		}
		return returnStatus;
	}

	public String editContAnalogsData()
	{
		if (oper.equalsIgnoreCase("del")) {
			deleteContAnalogRecords(id);
        }
		return SUCCESS;
	}
	
	private int deleteContAnalogRecords(String csvIdsString) {
...
//		StringBuffer strbufFaultsId = null;
		...
		try {
			logger.debug("ContAnalog Record ids "+csvIdsString);
			try
			{
				if (getStationId() == null)
				{
	...
	public String getAnalogStartTime() throws M9000Exception {
		return analogStartTime;
	}

	public void setAnalogStartTime(String analogStartTime) {
		this.analogStartTime = analogStartTime;
	}

	public String getAnalogEndTime() throws M9000Exception {
		return analogEndTime;
	}

	public void setAnalogEndTime(String analogEndTime) {
		this.analogEndTime = analogEndTime;
	}
