...
public class M9kDDRAnalogDataTableAction extends ActionSupport implements SessionAware,ServletRequestAware,ServletResponseAware {
	...
	static org.apache.logging.log4j.Logger logger = org.apache.logging.log4j.LogManager.getLogger(M9kDDRAnalogDataTableAction.class);
	...
	//JQGrid implementation
	  ...
	  private String id;
	  private static final String ltType = M9kConstants.ANALOG;
	  private String filters;
	  
	public M9kDDRAnalogDataTableAction() {
	...
	private String getLtrFileNames(String csvFaultsString) {
		...
		try {
			ltrFileNames = longTermDataDAO.getLongTermDataFileNames(Integer.parseInt(getStationId()), csvFaultsString.toString(), M9kConstants.ANALOG);
...
//		      total = (int) Math.ceil((double) records / (double) rows);
	...
	public String editDdrAnalogData()
	{
	...
	private int deleteDdrAnalog(String csvIdsString) {
...
//		StringBuffer strbufFaultsId = null;
		...
		try {
			logger.debug("Fault ids "+csvIdsString);
			try
			{
				if (getStationId() == null)
				{
				...
				messageProperties.addProperty("DATA_TYPE", M9kConstants.LTR_ANALOG_TYPE);
