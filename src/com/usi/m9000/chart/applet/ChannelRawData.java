...
public class ChannelRawData {
	...
	public void createChannelsWithData (int channelsCnt, short[][] channelsData, String[] channelsToDisplay)
	{
		ArrayList<Double>dataArray; 
		totalChannels = channelsToDisplay.length;
//		int channelNo;
//		for (int i = 0; i < channelsToDisplay.length; i++) {
//			channelNo = Integer.parseInt(channelsToDisplay[i].trim()) - 1;
//			System.out.println("\t\t\t\t\t\t\tchannel No: "+channelNo+"  channelsCnt "+channelsCnt);
//			if (channelNo >= 0 && channelNo < channelsCnt)
//			{
//		System.out.println("Channels data length "+channelsData.length);
		for (int i = 0; i<channelsData.length;i++)
		{
				dataArray = new ArrayList<Double>();
//				System.out.println("Total no of samples "+channelsData[i].length);
				for (int j = 0; j < channelsData[i].length; j++) {
					dataArray.add(new Double(channelsData[i][j]));
				}
				binaryChannelsList.add(dataArray);
